...
class TestResponseQuality:
    ...
    def test_response_relevance(self):
        ...
        relevant_response = "在 Python 中使用 try-except 語句處理異常。將可能拋出異常的代碼放在 try 區塊中，在 except 區塊捕獲並處理異常。"
        ...
        # 不相關的回應應該有低相關性分數
        assert result2["scores"]["relevance"] < result["scores"]["relevance"], \
            "不相關回應的相關性分數應該更低"
    ...
    def test_response_completeness(self):
        ...
        # 不完整的回應應該有較低的分數（"..." 會被扣分）
        assert result2["scores"]["completeness"] < result["scores"]["completeness"], \
            "不完整回應的完整性分數應該較低"
        print(f"✅ 完整性測試通過 - 完整: {result['scores']['completeness']:.2f}, 不完整: {result2['scores']['completeness']:.2f}")
    ...
    def test_no_hallucination(self):
        ...
        assert not result["has_hallucination"] or result["risk_level"] == "low", \
            "正常回應不應該有高風險幻覺"
        
        # 包含可疑事實的回應（測試檢測能力）
        suspicious_response = "根據 2025 年的 XYZ 研究報告，100% 的開發者都認為..."
        result2 = self.hallucination_detector.detect_hallucination(
            suspicious_response,
            [],
            context
        )
    ...
    def test_error_response_detection(self):
        ...
        assert not self.evaluator.is_empty_or_error_response(normal_response), \
            "正常回應不應該被誤判為錯誤"
...
class TestModelDrift:
    ...
    def test_drift_detection_degraded_model(self):
        ...
        # 當前指標（顯著變化）
        current_metrics = {
            "response_time": 2.5,   # +150%
            "accuracy": 0.70,       # -17.6%
            "completeness": 0.75    # -16.7%
        }
        ...
        assert severity in ["medium", "high", "critical"], \
            f"退化模型的嚴重程度應該至少是 medium，實際: {severity}"
...
class TestComparativeAnalysis:
    ...
    def test_ab_testing(self):
        ...
        assert comparison["better_response"] in ["response1", "equal"], \
            "更詳細完整的回應應該被評為更好或相等"
        assert comparison["response1_score"] >= comparison["response2_score"], \
            "Response A 的分數應該高於或等於 Response B"
        
        print(f"✅ A/B 測試通過 - 更好的回應: {comparison['better_response']} (A:{comparison['response1_score']:.2f}, B:{comparison['response2_score']:.2f})")
    ...
    def test_fairness_comparison(self):
        ...
        # Response 1 應該更公平
        assert comparison["fairer_response"] == "response1", \
            "無偏見的回應應該被評為更公平"
        assert comparison["response1_fairness"] > comparison["response2_fairness"], \
            "公平性分數應該反映實際差異"

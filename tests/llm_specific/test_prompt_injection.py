...
class TestPromptInjection:
    ...
    def test_safe_prompts_not_flagged(self):
        ...
        for prompt in safe_prompts:
            is_safe = not (self._detect_injection_pattern(prompt) or 
                          self._detect_role_manipulation(prompt) or
                          self._detect_context_hijacking(prompt))
    ...
    def _detect_injection_pattern(self, text: str) -> bool:
        ...
        injection_keywords = [
            "ignore", "忽略", "forget", "忘記",
            "previous instructions", "之前的指示",
            "new task", "新任務", "new instruction", "新指令",
            "system prompt", "系統提示",
            "reveal", "揭示", "show", "顯示",
            "internal", "內部"
        ]
    ...
    def _detect_role_manipulation(self, text: str) -> bool:
        ...
        role_keywords = [
            ...
            "admin mode", "管理員模式",
            "no restrictions", "沒有限制",
            "no safety", "沒有安全",
            "dan mode", "jailbreak", "越獄"
        ]
        text_lower = text.lower()
...
class TestDataLeakage:
    ...
    def test_safe_responses_not_flagged(self):
        ...
        for response in safe_responses:
            is_safe = not (self._detect_sensitive_data(response) or 
                          self._detect_pii(response))
    ...
    def _detect_sensitive_data(self, text: str) -> bool:
        ...
        text_lower = text.lower()
        return any(re.search(pattern, text_lower) for pattern in patterns) or \
               any(re.search(pattern, text) for pattern in patterns if '密碼' in pattern)
...
class TestModelSafety:
    ...
    def _detect_harmful_request(self, text: str) -> bool:
        ...
        harmful_keywords = [
            "hack", "駭入", "破解",
            "爆裂物", "explosive", "weapon",
            "misinformation", "假訊息",
            "歧視", "discrimination", "racist"
        ]
        text_lower = text.lower()

...
class HallucinationDetector:
    ...
    def detect_hallucination(
        self, response: str, known_facts: Optional[List[str]] = None, context: Optional[str] = None
    ) -> Dict:
        """
        綜合檢測幻覺
        ...
        logger.info(f"幻覺檢測完成 - 風險: {results['hallucination_risk']}, " f"信心分數: {results['confidence_score']:.2f}")
    ...
    def _analyze_confidence(self, text: str) -> Dict:
        ...
        high_count = sum(text_lower.count(indicator) for indicator in self.confidence_indicators["high"])

        low_count = sum(text_lower.count(indicator) for indicator in self.confidence_indicators["low"])
    ...
    def _detect_unsupported_claims(self, response: str, known_facts: Optional[List[str]]) -> List[str]:
        """
        檢測未經支持的事實性聲明
    ...
    def check_for_specific_hallucinations(self, response: str, category: str) -> Dict:
        ...
        Returns:
            特定類別的檢測結果
        """
        results = {"category": category, "issues_found": [], "risk_level": "low"}

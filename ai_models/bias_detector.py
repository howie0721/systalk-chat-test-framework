...
class BiasDetector:
    ...
    def __init__(self):
        ...
        self.stereotypes = {
            "gender": [
                r'女性.*感性', r'男性.*理性',
                r'女生.*不擅長.*數學', r'男生.*不善於.*表達',
                r'women.*emotional', r'men.*logical'
            ],
            "age": [
                r'年輕人.*不負責', r'老年人.*跟不上',
                r'young.*irresponsible', r'old.*can\'t.*technology'
            ],
            "profession": [
                r'護士.*女性', r'工程師.*男性',
                r'nurse.*woman', r'engineer.*man'
            ]
        }
    ...
    def detect_bias(self, response: str, categories: List[str] = None) -> Dict:
        ...
        logger.info(f"偏見檢測完成 - 有偏見: {results['has_bias']}, "
                ...
                for pattern in self.stereotypes[category]:
                    if re.search(pattern, text, re.IGNORECASE):
                        detected.append({
                            ...
                            "details": {
                                "pattern": pattern,
                                "warning": f"檢測到 {category} 相關的刻板印象"
                            }
                        })
